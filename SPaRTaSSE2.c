...
static inline int    sacInitRun(PROSAC_HEST_SSE2* restrict p,
                                ...
                                const float*               guessH,
                                float*                     finalH);
static inline void   sacFiniRun(PROSAC_HEST_SSE2* p);
static inline int    sacIsNREnabled(PROSAC_HEST_SSE2* p);
static inline int    sacIsRefineEnabled(PROSAC_HEST_SSE2* p);
static inline int    sacIsFinalRefineEnabled(PROSAC_HEST_SSE2* p);
static inline int    sacPhaseEndReached(PROSAC_HEST_SSE2* p);
static inline void   sacGoToNextPhase(PROSAC_HEST_SSE2* p);
static inline void   sacGetPROSACSample(PROSAC_HEST_SSE2* p);
static inline int    sacIsSampleDegenerate(PROSAC_HEST_SSE2* p);
static inline void   sacGenerateModel(PROSAC_HEST_SSE2* p);
static inline int    sacIsModelDegenerate(PROSAC_HEST_SSE2* p);
static inline void   sacEvaluateModelSPRT(PROSAC_HEST_SSE2* p);
static inline void   sacUpdateSPRT(PROSAC_HEST_SSE2* p);
static inline void   sacDesignSPRTTest(PROSAC_HEST_SSE2* p);
static inline int    sacIsBestModel(PROSAC_HEST_SSE2* p);
static inline int    sacIsBestModelGoodEnough(PROSAC_HEST_SSE2* p);
static inline void   sacSaveBestModel(PROSAC_HEST_SSE2* p);
static inline void   sacInitNonRand(double    beta,
                                    unsigned  start,
                                    unsigned  N,
                                    unsigned* nonRandMinInl);
static inline void   sacNStarOptimize(PROSAC_HEST_SSE2* p);
static inline void   sacUpdateBounds(PROSAC_HEST_SSE2* p);
static inline void   sacOutputModel(PROSAC_HEST_SSE2* p);
...
static inline int    sacInitRun(PROSAC_HEST_SSE2* restrict p,
	...
	if(sacIsNREnabled(p) && !hestSSE2EnsureCapacity(p, N, beta)){
		return 0;
	}
